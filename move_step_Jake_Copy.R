...
# This is not the original file and uses two move steps instead of one

for (move_iter in 1:2) { #JB: add two move steps 
  ...
  for (l in 1:N){
    ...
    ratio <- exp(pxs - px[l,M]) # JB: turn this quantity into new variable to check
    
    if (!is.finite(pxs) || !is.finite(px[l,M]) || !is.finite(ratio)) { #JB: move if any quantity is infinite when it should not be
      next
    }
    
    if (is.nan(pxs) == 0){
      if (u<min(1,ratio)){ #ratio put in here
        theta[l,,M] <- xs
  ...
  for (l in 1:N){
    for (q in 2:Rt){
     ...
     ratio <- exp(pxs - px[l,M]) #JB: same thing as above is done here
     
     if (!is.finite(pxs) || !is.finite(px[l,M]) || !is.finite(ratio)) { #JB: same as above
       next
     }
