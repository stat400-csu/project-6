...
run_smc_simulation <- function(input_N = 500, input_R = 0, input_I = 25, input_seed = 400) { #JD made original code into function
  ...
  for (M in 1:K){
    ...
    source('SMC_plot_Juliette_Copy.R', local = TRUE)
  }
  
  # Calculate Posterior Model Probabilities
  
  # Define the names of the models for clear output
   model_names <- c(
  "Beta Binomial type 2 functional response model",
  "Beta Binomial type 3 functional response model",
  "Binomial type 2 functional response model",
  "Binomial type 3 functional response model"
)
...
# Print the results to the console
  for (M in 1:K) {
  cat(paste(model_names[M], "probability:", posterior_model_prob[M], "\n"))
}
...
cat("Running Low Particle Simulation (N=30)...\n")
#  N = 30
results_N_lowest <- run_smc_simulation(input_N = 30, input_R = 0, input_seed = 23)
...
results_N_medium <- run_smc_simulation(input_N = 100, input_R = 0, input_seed = 88)
...
results_N_high <- run_smc_simulation(input_N = 1000, input_R = 0, input_seed = 166)
...
runtime_data <- data.frame(
  N = c(30, 100, 500, 1000), 
  Runtime_M_SS = c("0:16", "1:12", "3:33", "4:54")
)

# 2. Function to convert M:SS to total seconds
convert_to_sec <- function(time_str) {
  parts <- as.numeric(strsplit(time_str, ":")[[1]])
  return(parts[1] * 60 + parts[2])
}

# 3. Add the Runtime in seconds for plotting
runtime_data <- runtime_data %>%
  mutate(Runtime_sec = sapply(Runtime_M_SS, convert_to_sec))



# Find the maximum runtime value to calculate the needed expansion
max_runtime <- max(runtime_data$Runtime_sec)

# 4. Create the plot
runtime_plot <- ggplot(runtime_data, aes(x = factor(N), y = Runtime_sec, group = 1)) +
  
  geom_bar(stat = "identity", fill = "skyblue3") +
  
  # Add the exact time values on top of the bars
  geom_text(aes(label = Runtime_M_SS), vjust = -0.5, size = 5) +
  ...
  scale_y_continuous(
    limits = c(0, max_runtime * 1.15), # Set the max limit to 15% above the highest bar
    expand = expansion(mult = c(0, 0.05)) # Ensure the bottom starts at 0 and adds a little top padding
  ) +
  
  labs(
    title = "Computational Cost vs. Particle Count (N)",
    subtitle = "Runtime increases linearly with N",
...
```
```{r}
# R code to compute and display the percent increases
library(dplyr)
library(knitr)

runtime_data <- data.frame(
  N = c(30, 100, 500, 1000), 
  Runtime_sec = c(16, 72, 213, 294) # Use your calculated seconds
)

# Calculate the percent increase relative to the previous N value
percent_increase_data <- runtime_data %>%
  # Calculate the percentage change using lag()
  mutate(
    Prev_N = lag(N),
    N_Transition = paste0("N=", Prev_N, " -> N=", N),
    Percent_Increase = ((Runtime_sec - lag(Runtime_sec)) / lag(Runtime_sec)) * 100
  ) %>%
  ...
  mutate(
    `Percent Increase in Wait Time` = paste0(round(Percent_Increase, 1), "%")
  ) %>%
  select(
    Transition = N_Transition,
    `Percent Increase in Wait Time`
  )

# Print the final table
kable(percent_increase_data, 
      caption = "Sequential Increase in Computational Cost", 
      align = 'c')
```
