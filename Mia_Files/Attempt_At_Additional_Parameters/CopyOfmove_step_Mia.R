...
if(ESS[M] > tol){
  c <- cov(theta[,,M])
  if(any(is.na(c)) || any(!is.finit(c)) || det(c) == 0){
    cat('Warning: Invalid covariance matrix')
  } else {
...
cov_matrix <- cov_matrix + diag(1e-6, nrow(cov_matrix)) # MK: added to prevent singular matrix if this is the problem, remove if not
