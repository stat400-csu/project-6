...
#                                                                                               #                                                                                            #
#-----------------------------------------------------------------------------------------------#
# EDITED TO ACCOUNT FOR MORE PARAMETERS #
...
generate_data <- function(design, theta, observation_time, model){
  ...
  if (model %in% 1:2){
    ...
    phi <- exp(theta)
    phi_ode <- phi[, 1:4, drop = F] # MK: new parameters
    
    # Beta-binomial distribution
    v <-  solve_ode_Hollings(phi_ode, N0= design, observation_time = observation_time, model = model) # solve ode
    
    # Determine parameters
    design_matrix <- matrix(rep(design, times = nrow(theta)), byrow = T, nrow = nrow(theta))
    mu <- as.matrix((design_matrix - v)/design_matrix)
    
    lambda <- phi[,5] # MK: Lambda changed positions with new parameters
    alpha <- sweep(mu, 1, lambda, '/')
    beta <- sweep((1-mu), 1, lambda, '/')
