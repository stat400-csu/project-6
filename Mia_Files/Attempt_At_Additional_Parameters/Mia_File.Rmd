...
source('CopyOfgenerate_data.R')
source('CopyOfadditional_functions.R')
```
...
source('prior_sampling_w_extra_parameters.R')
...
for (i in 1:I){
  ...
  for (M in 1:K){
   ...
   #}
    ...
    if(is.na(sum_w) || is.nan(sum_w) || !is.finite(sum_w) || sum_w < 1e-8){
    ...
    if (ESS[M] < E){
      source('CopyOfresample_then_move.R')
    }
  }
    cat(" ", sep ="\n")  
  }
