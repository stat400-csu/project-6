---
title: "Mia_Fixed_Design"
output: html_document
---

# Author's Code, edits are noted with MK
...
source('Fixed_generate_data.R')
source('Fixed_additional_functions.R')
...
# MK: 6 - fixed experimental design points up front
...
if (R %in% 4:5){
  dataset <- read.table("Fixed_papanikolau_data.txt", sep = " ", col.names = c("N", "n")) %>% as.matrix
}
...
N <- 200 # number of particles for SMC
E <- N/2 # threshold for ESS for SMC
Nmin <- 1 # min value in the discrete design space
Nmax <- 100 # max value in the discrete design space
...
for (i in 1:I){
  ...
  if (R %in% c(0, 1, 2, 3, 5)){
    ...
    if  (R == 0){
      ...
      source('Fixed_parameter_estimation_utility.R') # select design points by maximising the parameter estimation utility
      source('Fixed_utility_plot.R') # plot utilty curve
      cat(" ", sep ="\n")  
    }
    
    
    else if (R == 2){
      source('Fixed_model_discrimination_utility.R') # select design points by maximising the model discrimination utility
      source('Fixed_utility_plot.R') # plot utility curve
      cat(" ", sep ="\n")  
    }
    
    
    else if (R == 3){
      source('Fixed_total_entropy_utility.R') # select design points by maximising the total entropy utility
      source('Fixed_utility_plot.R') # plot utility curve
      cat(" ", sep ="\n")  
    }
  ...
  else if(R == 6){ # MK: new experiment (fixed)
    if(i == 1){ # MK: Generate fixed design points
    data[,1] <- sample(Nmin:Nmax, I, replace = T)
    cat('Fixed Design: All designs chosen\n')
    cat('design points:\n')
    print(data[,1])
    }
    cat(paste('Using fixed design points:', data[i,1],'\n'))
    
    data[i,2] <- generate_data(data[i,1], parameter_set, observation_time = time, M_true)[[1]]
    
    cat(paste('Generated observation: n_eaten =', data[i,2],'\n'))
  }
  ...
  for (M in 1:K){
    ...
    if (ESS[M] < E){
      source('Fixed_resample_then_move.R')
    }
  }
...
cat('Design Summary')
cat('Design points used:\n')
print(data[i,1])
cat('\n Observations:\n')
print(data[i,2])
write.csv(data, paste0('results_R', R, '.csv'), row.names = F) # MK: Saving results
...
theta_R6 <- theta
log_Z_R6 <- log_Z
log_post_prob_R0 <- posterior_model_prob
