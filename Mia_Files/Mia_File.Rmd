...
source('Mia_Files/CopyOfgenerate_data.R')
source('Mia_Files/CopyOfadditional_functions.R')
```


Author's Code: ANY EDITS HAVE COMMENT WITH "MK"
```{r }
...
if (R %in% c(0:3,5)){
  ...
  Th <- 0.7
  lambda <- 0.5
  parameter_set <- matrix(c(beta, gamma, epsilon, Th, lambda), nrow = 1) %>% log
  M_true <- 1
}
...
source('Mia_Files/prior_sampling_w_extra_parameters.R')
...
for (i in 1:I){
  ...
  for (M in 1:K){
    ...
    log_like <- loglikelihood_Hollings(exp(theta[,,M]), data[i, , drop = F], time, models[M]) # MK: Checking for NA's
    
   # if (!is.numeric(log_like) || length(log_like) != N) {
   #   cat("ERROR: loglikehood function returned wrong shape for model:", models[M],"length:", length(log_like),"expected:", N,'\n')
   #   log_like <- rep(-Inf, N)
   #}
    log_like[!is.finite(log_like)] <- -Inf
    
    #check_noninf <- which(!is.finite(log_like))
    #if(length(check_noninf) > 0){
    #  log_like[check_noninf] <- -Inf
    #} # MK: Checking for NA's
    
    
    theta_exp <- exp(theta[,,M])
    cat(paste('Beta Range:', min(theta_exp[,1], na.rm = T), 'to', max(theta_exp[,1], na.rm = T), '\n'))
    cat(paste('Gamma Range:', min(theta_exp[,2], na.rm = T), 'to', max(theta_exp[,2], na.rm = T), '\n'))
    cat(paste('Epsilon Range:', min(theta_exp[,3], na.rm = T), 'to', max(theta_exp[,3], na.rm = T), '\n'))
    cat(paste('Th Range:', min(theta_exp[,4], na.rm = T), 'to', max(theta_exp[,4], na.rm = T), '\n'))
    cat(paste('Number of finite likelihoods:', sum(is.finite(log_like)), 'out of', N,'\n'))
    
    
    cat(paste('Is current normalized weight(before re-weighting) W[,M] NA anywhere?', any(is.na(W[,M])), 'any NaN?', any(is.nan(W[,M])),'\n'))
    
    #  Re-weight
    w[,M] = W[,M] * exp(log_like)
    
    cat(paste('Any new weights after updating w[,M] NA anywhere?', any(is.na(w[,M])), 'any NaN?', any(is.nan(w[,M])),'\n'))
    
    sum_w <- sum(w[, M])
    cat(paste('Sum of Weights:', sum_w, '\n'))
    
    if(is.na(sum_w) || is.nan(sum_w) || !is.finite(sum_w) || sum_w == 0){
      cat(paste('Weights are 0 or invalid for Model:', models[M], 'at iteration', i,'\n'))
      cat(paste('sum_w=', sum_w,'is.na = ', is.na(sum_w)))
      cat('Now resetting particles to prior dsn\n')
      if(models[M] %in% 1:2){
        theta[,,M] <- cbind(
      ...
      ESS[M] = N
      log_Z[M] = log_Z[M]
      cat(paste('Model', models[M], 'ESS:', ESS[M], '\n'))
      next
    }
    ...
    if(any(is.na(W[,M])) || any(is.nan(W[,M])) || any(!is.finite(W[,M]))){
      cat(paste('Error: NAs in normalized weights for model:', models[M]))
    bad_its <- which(is.na(W[,M]) | is.nan(W[,M]) | !is.finite(W[,M]))
    cat(paste('Number of invalid weights:', length(bad_its), '\n'))
      next
    }
    ...
    if (ESS[M] < E){
      source('Mia_Files/CopyOfresample_then_move.R')
    }
  }
    cat(" ", sep ="\n")  
  }
...
for (M in 1:K){
  ...
  source('Mia_Files/CopyOfSMC_plot.R')
  cat(" ", sep ="\n")  
}
