...
for (j in 1:length(periods_to_maturity)){
  stock = c()
  print(periods_to_maturity[j])
  for (i in 1:trials){
    stock[i] = generate_option_val(453.42, r, periods_to_maturity[j], sqrt(sigma_squared))
  }
  v[j] = mean(stock)
}
print(v)
print(paste("option value: ", mean(stock)))
print(paste("se of estimate: ", sd(stock)/(sqrt(trials))))
