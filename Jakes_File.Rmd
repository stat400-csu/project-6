...
Author's Code: ANY EDITS HAVE COMMENT WITH "JB"
```{r , cache = TRUE}

#True model TYPE II Binomial (two-step Move Step)
...
# Specify method of determining next design point
  ...
  for (i in 1:I){
   ...
   for (M in 1:K){
     ...
     if (ESS[M] < E){
        source('resample_then_move_Jake_Copy.R') #JB: uses my copy
      }
    }
  ...
  for (M in 1:K){
    ...
    if (ESS[M] != N){
      source('resample_then_move_Jake_Copy.R') #JB: uses my copu
   }
...
```{r , cache = TRUE}
#True model TYPE III Binomial (two-step Move Step)
...
#True model TYPE II Binomial (original)
...
# Specify method of determining next design point
  ...
  for (i in 1:I){
   ...
   for (M in 1:K){
     ...
     if (ESS[M] < E){
        source('resample_then_move_Jake_Copy2.R') #JB: uses my copy
      }
    }
  ...
  for (M in 1:K){
    ...
    if (ESS[M] != N){
      source('resample_then_move_Jake_Copy2.R') #JB: uses my copu
   }
...
```{r , cache = TRUE}
#True model TYPE III Binomial (original)
